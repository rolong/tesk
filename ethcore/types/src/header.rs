...
impl PartialEq for Header {
    fn eq(&self, c: &Header) -> bool {
        ...
        self.parent_hash == c.parent_hash
            ...
            && self.base_target == c.base_target
            && self.hash_rate == c.hash_rate
    }
}
...
impl Default for Header {
    fn default() -> Self {
        Header {
            ...
            base_target: Header::INITIAL_BASE_TARGET,
            hash_rate: U256::default(),
        }
    }
}

impl Header {
    /// Default base target value for genesis block
    const INITIAL_BASE_TARGET: u64 = 18325193796;

    /// Default EMPTY Block Generate Signature
    pub const EMPTY_GENERATE_SIGNATURE: [u8; 32] = [0; 32];
    ...
    ///Get the signature of the header.
    pub fn signature(&self) -> &H256 {
        &self.signature
    }

    ///Get the hash rate of the header.
    pub fn hash_rate(&self) -> &U256 {
        &self.hash_rate
    }

    ///Get the BaseTarget of the header.
    pub fn base_target(&self) -> u64 {
        self.base_target
    }

    /// Set the parent field of the header.
    ...
    /// set the signature of the header.
    pub fn set_signature(&mut self, a: H256) {
        change_field(&mut self.hash, &mut self.signature, a);
    }

    /// set the base_target of the header.
    pub fn set_base_target(&mut self, a: u64) {
        change_field(&mut self.hash, &mut self.base_target, a);
    }

    /// set the hash_rate of the header.
    pub fn set_hash_rate(&mut self, a: U256) {
        change_field(&mut self.hash, &mut self.hash_rate, a);
    }

    /// Place this header into an RLP stream `s`, optionally `with_seal`.
    fn stream_rlp(&self, s: &mut RlpStream, with_seal: Seal) {
        if let Seal::With = with_seal {
            s.begin_list(16 + self.seal.len());
        } else {
            s.begin_list(16);
        }
        ...
        s.append(&self.base_target);
        s.append(&self.hash_rate);
...
impl Decodable for Header {
    fn decode(r: &Rlp) -> Result<Self, DecoderError> {
        let mut blockheader = Header {
            ...
            signature: r.val_at(13)?,
            base_target: r.val_at(14)?,
            hash_rate: r.val_at(15)?,
