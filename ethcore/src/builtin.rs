...
//static rpc_port: Arc<Cell<AtomicUsize<u16>>> = Arc::new(Cell::new(AtomicUsize::new((0))));
//static rpc_port: Arc<Mutex<u16>> = Arc::new(Mutex::new(0));
//static rpc_port: u16 = 0;

static mut RPC_PORT: Option<u16> = None;

pub fn set_rpc_port(port: u16) {
    println!("set_rpc_port {}", port);
    unsafe {
        RPC_PORT = Some(port);
        println!("set_rpc_port after: {:?}", RPC_PORT);
    }
}

fn get_rpc_port() -> Option<u16> {
    unsafe {
        let p = RPC_PORT;
        return p;
    }
...
pub fn ethereum_builtin(name: &str) -> Box<dyn Impl> {
    match name {
        ...
        "hashofrate" => Box::new(HashOfRate) as Box<dyn Impl>,
...
pub struct EthGetBlockByNumberParam {
    pub jsonrpc: String,
    pub id: u64,
    pub method: String,
...
#[derive(Debug)]
struct HashOfRate;
/// how you can use the hashofrate methods in assembly language
///
/// ```rust
/// pragma   solidity >=0.4.22 <0.6.0;
///
/// contract Test4 {
///     uint256 public daemon = 0;
///     bytes32[1]  public   a;
///     function  getblockhashrate(uint256 blocknumber) public returns  ( bytes32[1]  memory){
///
///        bytes32[1]  memory input;
///
///        bytes32[1]  memory output;        bytes32[1]  memory output;
///
...
///            }
///        }
///        a = output; // hexdecimal output
///        daemon = uint256(output[0]); // decimal output
///        return output;
///    }
/// }
/// ```
impl Impl for HashOfRate {
    /// HashOfRate: parameter
    /// input: block.number
    /// output: should the block.hash_rate
    fn execute(&self, input: &[u8], output: &mut BytesRef) -> Result<(), Error> {
        let block_number = {
            let (int_bytes, _) = input.split_at(std::mem::size_of::<u64>());
            u64::from_be_bytes(int_bytes.try_into().unwrap())
        };
        let param = EthGetBlockByNumberParam {
            method: "eth_getBlockByNumber".to_string(),
            id: 1,
            jsonrpc: "2.0".to_string(),
            params: (format!("{:#x}", block_number), false),
        };

        let client = reqwest::Client::new();
        let url = match get_rpc_port() {
            Some(port) => format!("http://127.0.0.1:{}", port),
            None => {
                trace!(target: "builtin", "RPC port not initialized");
                return Err(Error::from("RPC port not initialized"));
            }
        };
        let resp = client.post(url.as_str())
            .json(&param)
            .send().map_err(|e| {
            trace!(target: "builtin", "eth_getBlockByNumber: {:?}", e);
            Error::from("get block hash_rate fails")
        })?
            .text().map_err(|e| {
            trace!(target: "builtin", "eth_getBlockByNumber: text: {:?}", e);
            Error::from("get block hash_rate get text fails")
        })?;
        let resp: EthGetBlockByNumberResponse = serde_json::from_str(&resp).map_err(|e| {
            trace!(target: "builtin", "eth_getBlockByNumber: parse: {:?}", e);
            Error::from("get block hash_rate parse fails")
        })?;

        let original_string: &str = string_to_static_str(resp.result.hash_rate);  // take a full slice of the string
        let without_prefix = original_string.trim_start_matches("0x");
        let str_u256 = U256::from(without_prefix);
        let mut input_tmp = [0_u8; 32];
        str_u256.to_big_endian(&mut input_tmp[..]);
        output.write(0, &input_tmp[..]);
        Ok(())
    }
}
