...
impl Miner {
        ...
        C: BlockChain + CallContract + BlockProducer + Nonce + Sync,
    {
        ...
        let block = match open_block.close() {
            Ok(block) => block,
            Err(err) => {
                error!(target: "miner", "Closing the block failed with error {:?}. This is likely an error in chain specificiations or on-chain consensus smart contracts.", err);
                return None;
            }
        };
...
impl miner::MinerService for Miner {
    ...
    //(H256, BlockNumber, u64, U256 , H256 ,u64 ,&[Bytes], Address)
    fn work_package<C>(&self, chain: &C) -> Option<Header> where
        C: BlockChain + CallContract + BlockProducer + SealedBlockImporter + Nonce + Sync,
    {
//		if self.engine.seals_internally().is_some() || !self.engine.name().eq("PoC") {
//			return None;
//		}
        ...
        self.sealing.lock().queue.use_last_ref().map(|b| {
            ...
            //(header.hash(), header.number(), header.timestamp(), *header.difficulty() ,*header.signature(),header.base_target(),header.seal(),*header.author())
        })
    }
    ...
    fn submit_seal(&self, block_hash: H256, seal: Vec<Bytes>) -> Result<SealedBlock, Error> {
        let result =
            ...
            self.sealing.lock().queue.get_used_if(
