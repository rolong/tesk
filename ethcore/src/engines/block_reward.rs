...
impl BlockRewardContract {
    ...
    pub fn reward(
        ...
        // Skhash of PoC consensus apply block rewards in block_reward contract
        if output.len() < 1 {
            let null_value = vec![
                ("0000000000000000000000000000000000000000".into(), U256::from(0)),
                ("0000000000000000000000000000000000000000".into(), U256::from(0)),
                ("0000000000000000000000000000000000000000".into(), U256::from(0)),
            ];
...
mod test {
    ...
    fn block_reward_contract() {
        ...
        assert!(block_reward_contract.reward(&U256::from(1), &vec![], &mut call).unwrap().is_empty());
        ...
        let hash_rate = U256::from(0);
        let rewards = block_reward_contract.reward(&hash_rate, &beneficiaries, &mut call).unwrap();
        let expected = vec![
