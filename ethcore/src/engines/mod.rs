...
pub use self::skhash::{Skhash, SkhashSeal};

pub mod nonce_check;
pub mod miner_verify;
...
impl fmt::Display for EngineError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let msg = match *self {
            ...
            PoCEngineNoSigner => format!("PoC must have a sginer but no signer register in Engine"),
            PoCMissingSignature => format!("Extra data is missing signature"),
            FailedMinerVerify(ref msg) => format!("Failed to Miner verify from contract: {}", msg),
        };
...
/// A consensus mechanism for the chain. Generally one of proof-of-work, proof-of-capacity or proof-of-stake-based.
...
pub trait Engine<M: Machine>: Sync + Send {
    ...
    /// Allow mutating the header during seal generation. Currently only used by Clique and Skhash.
    ...
    /// Returns next generation_signature.
    /// It is currently only used in Skhash consensus.
    fn next_generation_signature(&self, _header: &Header) -> H256 {
        Default::default()
    }
