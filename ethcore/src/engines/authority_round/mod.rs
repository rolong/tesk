...
impl EpochManager {
    ...
    fn zoom_to(&mut self, client: &dyn EngineClient, machine: &EthereumMachine, validators: &dyn ValidatorSet, header: &Header) -> bool {
        ...
        for finalized_hash in finalized {
            if let Some(pending) = transition_store(*finalized_hash) {
                ...
                let mut finality_proof: Vec<_> = itertools::repeat_call(move || {
                    chain(hash).and_then(|header| {
                        hash = *header.parent_hash();
                        if header.number() == 0 { None } else { Some(header) }
                    })
                })
                    .while_some()
