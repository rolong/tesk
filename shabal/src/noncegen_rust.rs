    ...
    for n in 0..local_nonces {
            ...
            .step_by(HASH_SIZE)
        {
            ...
            if i % 64 == 0 {
                ...
                let hash = &shabal256_hash_fast(&buffer[i..NONCE_SIZE], &t1);
                buffer[i - HASH_SIZE..i].clone_from_slice(hash);
            } else {
                // last msg = 256 bit data + seed + termination
                let hash = &shabal256_hash_fast(&buffer[i..NONCE_SIZE], &t2);
                buffer[i - HASH_SIZE..i].clone_from_slice(hash);
            }
        }
        ...
        for i in (HASH_SIZE..=NONCE_SIZE - HASH_CAP).rev().step_by(HASH_SIZE) {
            let hash = &shabal256_hash_fast(&buffer[i..i + HASH_CAP], &t3);
            buffer[i - HASH_SIZE..i].clone_from_slice(hash);
        }

        // generate final hash
        final_buffer.clone_from_slice(&shabal256_hash_fast(&buffer[0..NONCE_SIZE], &t1));
