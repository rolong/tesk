...
use v1::types::{
    BlockNumber, Bytes, CallRequest, EthAccount, Filter, FilterChanges, Index, RichBlock,
};
...
/// Eth mining interface. This is currently only used in PoC consensus.
#[rpc]
pub trait EthMining {
    /// Used for getting mining info.
    #[rpc(name = "eth_getMiningInfo")]
    fn get_mining_info(&self) -> Result<MiningInfo>;

    /// Used for submitting a nonce.
    #[rpc(name = "eth_submitNonce")]
    fn submit_nonce(&self, Nonce) -> Result<SubmitNonceResponse>;
}
