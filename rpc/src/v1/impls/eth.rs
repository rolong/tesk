...
use ethcore::client::{
    BlockChainClient, BlockId, Call, EngineInfo, ProvingBlockChainClient, StateClient, StateInfo,
    StateOrBlock, TransactionId, UncleId,
};
use ethcore::engines::SkhashSeal;
...
use shabal::{
    calculate_scoop, decode_gensig, find_best_deadline_rust, noncegen_rust, shabal256_hash_fast,
};
use sync::SyncProvider;
use types::encoded;
use types::filter::Filter as EthcoreFilter;
use types::header::Header;
use types::transaction::{LocalizedTransaction, SignedTransaction};
...
use v1::helpers::dispatch::{default_gas_price, FullDispatcher};
use v1::helpers::{self, errors, fake_sign, limit_logs};
use v1::traits::Eth;
...
use v1::types::{
    ...
    Filter, Index, Log, Receipt, RichBlock, StorageProof, SyncInfo, SyncStatus, Transaction, Work,
};
            ...
            (Some(block), Some(total_difficulty)) => {
                ...
                Ok(Some(RichBlock {
                    inner: Block {
                        ...
                        base_target: view.base_target().into(),
                        hash_rate: view.hash_rate().into(),
    ...
    fn uncle(&self, id: PendingUncleId) -> Result<Option<RichBlock>> {
        ...
        let block = RichBlock {
            inner: Block {
                ...
                base_target: uncle.base_target().into(),
                hash_rate: uncle.hash_rate().into(),
    ...
    EM: ExternalMinerService + 'static,
{
    ...
    fn work(&self, no_new_work_timeout: Option<u64>) -> Result<Work> {
        ...
        info!(target: "miner", "Syncing. give work begin.");
        // check if we're still syncing and return empty strings in that case
        {
            ...
            if sync_status.is_snapshot_syncing() || total_queue_size > MAX_QUEUE_SIZE_TO_MINE_ON {
                info!(target: "miner", "Syncing. Cannot give any work.");
        ...
        let header = self.miner.work_package(&*self.client).ok_or_else(|| {
            warn!(target: "miner", "Cannot give work package - engine seals internally.");
            errors::no_work_required()
        })?;
        let number = header.number();
        let target = ethash::difficulty_to_boundary(&header.difficulty());
        ...
        let now = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        if no_new_work_timeout > 0 && header.timestamp() + no_new_work_timeout < now {
