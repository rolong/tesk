...
use seek_chain::{PubSubSession, RunningClient};
...
pub unsafe extern fn parity_config_from_cli(
	...
	panic::catch_unwind(|| {
		*output = ptr::null_mut();
		...
		match seek_chain::Configuration::parse_cli(&args) {
			Ok(mut cfg) => {
...
pub unsafe extern fn parity_config_destroy(cfg: *mut c_void) {
	let _ = panic::catch_unwind(|| {
		let _cfg = Box::from_raw(cfg as *mut seek_chain::Configuration);
	});
}
...
pub unsafe extern fn parity_start(cfg: *const ParityParams, output: *mut *mut c_void) -> c_int {
	panic::catch_unwind(|| {
		*output = ptr::null_mut();
		...
		let logger = Arc::from_raw(cfg.logger as *mut seek_chain::RotatingLogger);
		let config = Box::from_raw(cfg.configuration as *mut seek_chain::Configuration);
		...
		let action = match seek_chain::start(*config, logger, on_client_restart_cb, || {}) {
			Ok(action) => action,
			Err(_) => return 1,
		};

		match action {
			seek_chain::ExecutionAction::Instant(Some(s)) => { println!("{}", s); 0 },
			seek_chain::ExecutionAction::Instant(None) => 0,
			seek_chain::ExecutionAction::Running(client) => {
...
pub unsafe extern fn parity_set_logger(
	...
	let mut logger_cfg = seek_chain::LoggerConfig::default();
	...
	*logger = Arc::into_raw(seek_chain::setup_log(&logger_cfg).expect("Logger initialized only once; qed")) as *mut _;
}
