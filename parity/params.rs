...
impl str::FromStr for SpecType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let spec = match s {
			...
			"dev" => SpecType::Dev,
			"skhash" | "seekchain" => SpecType::Skhash,
...
impl SpecType {
	pub fn spec<'a, T: Into<SpecParams<'a>>>(&self, params: T) -> Result<Spec, String> {
		...
		match *self {
			...
			SpecType::Skhash => Ok(ethereum::new_skhash(params)),
...
mod tests {
	...
	fn test_spec_type_parsing() {
		...
		assert_eq!(SpecType::Skhash, "skhash".parse().unwrap());
	...
	fn test_spec_type_display() {
		...
		assert_eq!(format!("{}", SpecType::Skhash), "skhash");
