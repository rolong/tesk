...
impl Configuration {
	...
	///
	/// ```
	/// let _cfg = seek_chain::Configuration::parse_cli(&["--light", "--chain", "kovan"]).unwrap();
	/// ```
	...
	fn init_reserved_nodes(&self) -> Result<Vec<String>, String> {
		...
		match self.args.arg_reserved_peers {
			Some(ref path) => {
				...
				let mut node_file = File::open(&path).map_err(|e| format!("Error opening reserved nodes file: {}, filename: {}", e, &path))?;
